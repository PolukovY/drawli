...
Кожен білд отримує `VITE_BUILD_ID` (у CI — sha коміту). Ним іменуються рантайм-кеші
(`drawli-exercises-<id>`, `drawli-catalogue-<id>`), тож новий деплой починає з порожнього
кешу, а не доїдає вчорашні малюнки. Кеші попередніх білдів прибирає `pruneStaleCaches()`
на старті. У «Налаштуваннях» є кнопка «Оновити застосунок» (`refreshApp()`): чистить усі
кеші, знімає service worker і перезавантажує сторінку. IndexedDB не чіпається — малюнки,
зірки й налаштування лишаються.
